...
+ (id)pickerWithDate:(NSDate *)date type:(NSNumber*)type useMinMax:(BOOL)useMinMax selectedBlock:(AIDatePickerDateBlock)selectedBlock cancelBlock:(AIDatePickerVoidBlock)cancelBlock
{
    ...
    {
        calender = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierIslamicCivil];
    }
    ...
    if(useMinMax)
    {
        ...
        [comps setYear:-10];
        NSDate *maxDate = [calender dateByAddingComponents:comps toDate:currentDate options:0];
        
        [comps setYear:-117];
        [comps setMonth:8];
        NSDate *minDate = [NSDate dateFromString:@"01-05-1900" withFormat:@"dd-MM-yyyy"];//[calender dateByAddingComponents:comps toDate:currentDate options:0];
        
        [ datePickerController.datePicker setMaximumDate:maxDate];
        [ datePickerController.datePicker setMinimumDate:minDate];
    }
...
- (id)init
{
    ...
    _datePicker.calendar = calender;
    
    NSLocale *locale;
    if([[NLCommon getDeviceLanguage] isEqualToString:@"ar"])
    {
        locale = [[NSLocale alloc]
                  initWithLocaleIdentifier:@"ar_US"];
    }
    else
    {
        locale = [[NSLocale alloc]
